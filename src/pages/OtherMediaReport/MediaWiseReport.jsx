  ...
  const prepareYearlyData = bookings => {
    ...
    bookings.forEach(booking => {
      ...
      if (filter === 'yearly') {
        ...
        // Reset quarterly summary if needed
        if (!quarterlySummaries[quarter]) {
          quarterlySummaries[quarter] = {
            ownedSiteRevenue: 0,
            tradedSiteRevenue: 0,
        ...
        if (clientData) {
          quarterlySummaries[quarter].ownedSiteRevenue += Number.isFinite(
            clientData.ownedSiteRevenue,
          )
            ? clientData.ownedSiteRevenue
            : 0;
          quarterlySummaries[quarter].tradedSiteRevenue += Number.isFinite(
          ...
          quarterlySummaries[quarter].tradedMargin += Number.isFinite(clientData.tradedMargin)
            ...
            : 0;
          quarterlySummaries[quarter].operationalCosts.electricity += Number.isFinite(
            clientData.operationalCosts.electricity,
          )
            ? clientData.operationalCosts.electricity
            : 0;
          quarterlySummaries[quarter].operationalCosts.licenseFee += Number.isFinite(
            clientData.operationalCosts.licenseFee,
          )
            ? clientData.operationalCosts.licenseFee
            : 0;
          quarterlySummaries[quarter].operationalCosts.rental += Number.isFinite(
            clientData.operationalCosts.rental,
          )
            ? clientData.operationalCosts.rental
            : 0;
          quarterlySummaries[quarter].operationalCosts.misc += Number.isFinite(
            clientData.operationalCosts.misc,
          )
            ? clientData.operationalCosts.misc
            : 0;
          quarterlySummaries[quarter].tradedPurchaseCost += Number.isFinite(
            clientData.tradedPurchaseCost,
          )
            ? clientData.tradedPurchaseCost
            : 0;
          quarterlySummaries[quarter].grossRevenueOwned += Number.isFinite(
            clientData.grossRevenueOwned,
          )
            ? clientData.grossRevenueOwned
            : 0;
          quarterlySummaries[quarter].grossRevenueTraded += Number.isFinite(
        ...
        console.log('Quarterly Summaries After Update:', quarterlySummaries[quarter]);
      }
